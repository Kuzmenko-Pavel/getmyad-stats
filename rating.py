...
class GetmyadRating(object):
    ...
    def importWorkerData(self, db, pool):
        ...
        filds = {'_id': True, 'id': True, 'dt': True, 'title': True, 'inf':True, 'campaignId':True, 'campaignTitle':True, 'id_int':True, 'inf_int':True, 'campaignId_int':True, 'retargeting':True}
                ...
                try:
                    for x in cursor:
                        try:
                            ...
                            key_rating = (x['id'], x['title'], x['inf'].lower(), dt, x['campaignId'].lower(), x['campaignTitle'],\
                                    x['id_int'], x['inf_int'], x['campaignId_int'])
                            campaign_key_rating = (x['inf'].lower(), x['campaignId'].lower(), x['inf_int'], x['campaignId_int'])
        ...
        for key,value in campaign_rating_buffer.iteritems():
            try:
                db.campaign.rating.update({'adv': key[0], 'adv_int': key[2], 'campaignId': key[1],'campaignId_int': key[3]},
    ...
    def createOfferRating(self, db):
        msg={}
        campaignIdList = [ x['guid'] for x in db.campaign.find({"showConditions.retargeting" : False, "status":"working" })]
    ...
    def delete_old_rating_stats(self, db):
        ...
        y = db.offer.update({'$or':[{'impressions':{'$lte':0}},{'impressions':{'$gte':1500}},{'impressions':{'$exists':False}}]},
                            {'$set': {'impressions': 0,
                                      'clicks': 0 }}, multi=True, w=1)
        #Сбрасуем показы и клики в товарах по рекламному блоку
        z = db.stats_daily.rating.update({'$or':[{'impressions':{'$lte':0}},{'impressions':{'$gte':1500}},{'impressions':{'$exists':False}}]},
                                         {'$set': {'impressions': 0,
                                          'clicks': 0 }}, multi=True, w=1)
